...
class InforequestDraftTest(InforequestsTestCaseMixin, TestCase):
    ...
    def test_modified_field_is_autogenerated_when_creating_new_instance(self):
        dt = local_datetime_from_local(u'2014-10-05 10:33:00')
        draft = self._create_inforequest_draft(modified=dt)
        self.assertAlmostEqual(draft.modified, utc_now(), delta=datetime.timedelta(milliseconds=10))

    def test_modified_field_is_regenerated_when_saving_existing_instance(self):
        timewarp.jump(local_datetime_from_local(u'2014-10-05 10:33:00'))
        draft = self._create_inforequest_draft()
        dt = local_datetime_from_local(u'2014-10-05 11:44:00')
        timewarp.jump(dt)
        draft.save()
        draft = InforequestDraft.objects.get(pk=draft.pk)
        self.assertAlmostEqual(draft.modified, dt, delta=datetime.timedelta(milliseconds=100))
