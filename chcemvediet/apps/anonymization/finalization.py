...
def finalize_using_mock(attachment_finalization):
    ...
    with open(finalized, u'rb') as file:
        AttachmentFinalization.objects.create(
            attachment=attachment_finalization.attachment,
            successful=True,
...
def finalize_using_libreoffice(attachment_finalization):
    if settings.MOCK_LIBREOFFICE:
        finalize_using_mock(attachment_finalization)
        return
    ...
    try:
        ...
        with temporary_directory() as directory:
            filename = os.path.join(directory,
                                    u'file' + guess_extension(attachment_finalization.content_type)
                                    )
            shutil.copy2(attachment_finalization.file.path, filename)
            p = subprocess32.run(
        ...
        cron_logger.error(u'Finalizing attachment using libreoffice has failed: {}\n An '
                          u'unexpected error occured: {}\n{}'.format(
                                  attachment_finalization, e.__class__.__name__, trace))
